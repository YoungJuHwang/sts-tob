...
var Event = {
		event : function(project) {
			$("#box").load(project + "/event/Event.do");
		},
		ranking : function(project) {
			var arr = [];
			$.getJSON(project + '/event/Event.do?page=event_form1',function(data){
				var rank = '<div class="wrap auto"><div class="header"></div><div class="big"><div class="big_1"><div class="cate">'
				...
				+'<div class="sm_2_3"><form action="">이벤트 검색:<input type="text" name="검색"><input type="submit" value="검색"></form></div></div></div></div></div></div></div>'
				+'<div class="big_2">'
				$.each(data, function(index, value) {
				rank += '<div class="img"><img id='this.evtId+' src="../images/'+this.evtId+'.jpg" alt="" width="150" height="100"><div class="desc"><br /><br />'
						+'<a class="highlight" href="#">'+this.evtName+'</a>'+this.fromDt+''+toDt+'</div></div></div>'
					arr.push(this.evtId);
